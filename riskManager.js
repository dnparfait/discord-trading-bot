...
function calculateLotSize({ balance, riskPercent, stopLossPips, symbol = 'EURUSD', pipValue = null, minLot = 0.01, maxLot = 5.0 }) {
  if (!stopLossPips || stopLossPips <= 0) throw new Error('Stop Loss manquant ou invalide — trade rejeté');
  if (!balance || balance <= 0) throw new Error('Balance invalide');
...
function checkDrawdown({ currentBalance, initialBalance, maxDrawdownPct }) {
  const drawdown = ((initialBalance - currentBalance) / initialBalance) * 100;
  if (drawdown >= maxDrawdownPct) throw new Error(`Drawdown maximum atteint (${drawdown.toFixed(1)}%) — bot suspendu`);
  return parseFloat(drawdown.toFixed(2));
}

function calcStopLossPips(entryPrice, stopLoss, symbol = 'EURUSD') {
...
module.exports = { getPipValue, calculateLotSize, checkDrawdown, calcStopLossPips };
